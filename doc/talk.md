...
## 会话：2026-05-20（Chrome 历史入口消失）

### 用户反馈

用户反馈播放历史界面入口不见了，不同浏览器效果不同：Firefox 有，Chrome 没有；使用 `Ctrl + Shift + R` 强制刷新也无效。

### 排查

- 播放历史功能本身仍存在：`index.html` 中 `btn-history` 和 `history-overlay` 都还在，前端也仍消费 `/api/history`。
- 差异来自布局而不是缓存：`HIST` 按钮位于页面最底部 footer，Chrome / Firefox 的字体渲染、地址栏高度和视口计算不同，导致 footer 在 Chrome 被挤到可视区域外。
- 强制刷新只能解决缓存问题，不能解决元素被布局推到视口外的问题。

### 修复

- `public/index.html`：新增固定的 `HIST` 快捷入口 `btn-history-fab`，即使底部 footer 被浏览器视口挤出，也能打开播放历史。
- `public/app.js`：让固定 `HIST` 快捷入口复用原有历史面板开关逻辑，并同步 active 状态。
- `public/style.css`：为固定 `HIST` 快捷入口增加跨浏览器可见样式，避免影响 DJ 区、输入框和原 footer。
- 新增 `tests/footer-nav-layout.test.js`，覆盖原 footer 历史按钮、固定 `HIST` 快捷入口、JS 绑定和固定定位样式。
